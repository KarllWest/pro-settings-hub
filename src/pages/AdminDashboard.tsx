...
import { 
  ...
  Edit2, Trash2, ExternalLink, Filter, Loader2 
} from 'lucide-react';
        ...
        <div className="p-8 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 bg-white/[0.02]">
          <div className="relative w-full max-w-md group">
            ...
            <input 
              ...
              className="w-full bg-slate-950 border border-white/5 rounded-2xl py-3.5 pl-12 pr-4 text-sm focus:outline-none focus:border-yellow-400/40 transition-all text-white placeholder:text-slate-700 shadow-inner"
            />
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredPlayers.map((player) => (
                <tr key={player.id} className="hover:bg-white/[0.01] transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <img 
                        ...
                        className="w-12 h-12 rounded-xl object-cover border border-white/10 shadow-lg grayscale group-hover:grayscale-0 transition-all duration-500" 
                        alt="" 
                      />
                      <div>
                        <p className="font-black text-white group-hover:text-yellow-400 transition-colors uppercase italic text-base">{player.nickname}</p>
                  ...
                  <td className="px-8 py-5">
                    <span className="text-sm font-black text-slate-400 uppercase italic group-hover:text-slate-200 transition-colors">
...
function StatCard({ icon, label, value, color }: StatCardProps) {
  return (
    <div className="glass-panel p-8 rounded-[2rem] hover:border-white/10 transition-all group">
