...
import { Helmet } from 'react-helmet-async'; // ВАЖЛИВО
...
export default function GamePage({ game }: GamePageProps) {
  ...
  return (
    ...
    >
      {/* 1. SEO: Динамічний заголовок тут */}
      <Helmet>
        <title>{titles[game]} Pro Settings | KeyBindy</title>
      </Helmet>
            ...
            >
              {filteredPlayers.length > 0 ? (
                filteredPlayers.map((player, index) => (
                    ...
                    >
                      <Link to={`/player/${player.id}`} className="block h-full group">
